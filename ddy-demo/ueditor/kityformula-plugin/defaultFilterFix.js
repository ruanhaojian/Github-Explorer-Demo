  ...
  me.addInputRule(function(root) {
    ...
    root.traversal(function(node) {
      if (node.type == 'element') {
        if (
          !UE.dom.dtd.$cdata[node.tagName] &&
          me.options.autoClearEmptyNode &&
          UE.dom.dtd.$inline[node.tagName] &&
          !UE.dom.dtd.$empty[node.tagName] &&
          (!node.attrs || UE.utils.isEmptyObject(node.attrs))
        ) {
        ...
        switch (node.tagName) {
          ...
          case 'caption':
            if (!node.children || !node.children.length) {
              node.appendChild(
                UE.browser.ie11below ? UE.uNode.createText(' ') : UE.uNode.createElement('br')
              );
            }
            break;
          case 'table':
